...
{% image "https://cdn-images-1.medium.com/max/3860/1*i4DqT6krOCc5JH9Fk9CpmQ.png", "Perbandingan sintaks OCaml, Reason lama, dan Reason terbaru." %}_Perbandingan sintaks OCaml, Reason lama, dan Reason terbaru._
...
{% image "https://cdn-images-1.medium.com/max/5760/1*nlpG7UlWXmAhIjF8FbgdyA.png", "Fungsi add menerima dua buah parameter integer dan fungsi add akan me-return integer juga." %}_Fungsi add menerima dua buah parameter integer dan fungsi add akan me-return integer juga._
...
{% image "https://cdn-images-1.medium.com/max/4612/1*GvIibB8iZCIUA2q879Bedg.png", "Reason mampu menebak records sesuai properti yang terkandung didalamnya." %}_Reason mampu menebak tipe records sesuai properti yang terkandung didalamnya._
...
{% image "https://cdn-images-1.medium.com/max/3596/1*3VpX4PwJLhVS7e5ag_droQ.png", "Pesan kesalahan yang detil dan manusiawi." %}_Pesan kesalahan yang detil dan manusiawi._
...
{% image "https://cdn-images-1.medium.com/max/2000/1*zTMnImlX1yGVpZmAjp1WhA.png", "Bukan pesan kesalahan yang kita dapatkan, akan tetapi bug yang akan muncul jika di JavaScript" %}
...
{% image "https://cdn-images-1.medium.com/max/2000/1*6HeGFhbOkfMNG9saWV-FAA.png", "Mendeteksi Typo." %}_Mendeteksi Typo._
...
{% image "https://cdn-images-1.medium.com/max/3888/1*eqqXemwsvNxetgfUFemx1Q.png", "Reason ‘meminjam’ beberapa alat bantu yang sudah ada di ekosistem JavaScript" %}_Reason ‘meminjam’ beberapa alat bantu yang sudah ada di ekosistem JavaScript_
...
{% image "https://cdn-images-1.medium.com/max/3840/1*vktrkdYUCyeJQlTfvl7TUA.png", "Grid yang diadaptasi dari talk Jared Forsyth" %}
_Grid yang diadaptasi dari talk Jared Forsyth ([https://jaredforsyth.com/](https://jaredforsyth.com/))_
...
{% image "https://cdn-images-1.medium.com/max/3212/1*zTSKdzAuvwzi61RQnUV47w.png", "Interopabilitas Reason dengan JavaScript masih cukup dekat." %}_Interopabilitas Reason dengan JavaScript masih cukup dekat._
...
{% image "https://cdn-images-1.medium.com/max/4148/1*gEXMwYk-7bq0LcPYBpHb8A.png", "Disapa oleh Leader Komunitas ketika baru join grup discord." %}_Disapa oleh Leader Komunitas ketika baru join grup discord._
...
{% image "https://cdn-images-1.medium.com/max/2240/1*RsJilTLJ3joLfNEZG9C1Gw.png", "Penggunaan mutable variable dan imperative loop." %}_Penggunaan mutable variable dan imperative loop._
...
{% image "https://cdn-images-1.medium.com/max/2200/1*HCeyWrc1lpvI30BpCB2dbQ.png", "Isi file package.json" %}_Isi file package.json_
...
{% image "https://cdn-images-1.medium.com/max/2200/1*T-UkFOCEFuCCYB5WOTm6yA.png", "Isi file bsconfig.json" %}_Isi file bsconfig.json_

Sekarang mari kita coba buka file src/Demo.re . Isinya kodenya cukup sederhana, sebuah pesan Hello World.

{% image "https://cdn-images-1.medium.com/max/2240/1*G2-tutUVTQOX1K17E-2CCw.png", "Placeholder kode Demo.re" %}_Placeholder kode Demo.re_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*p8VliE5_MXpqGOWqbtKKJw.png", "Hasil kompilasi BuckleScript" %}_Hasil kompilasi BuckleScript_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*8C-dM0qfhXlGmoH7vSExkA.png", "Load JavaScript file ke HTML" %}_Load JavaScript file ke HTML_

Dan kita buka file tersebut di browser dan jangan lupa buka _chrome console_ untuk melihat hasilnya.

{% image "https://cdn-images-1.medium.com/max/3112/1*8ySipLVTgIAVtXWMgtZNRQ.png", "Dijalankan di browser" %}_Dijalankan di browser_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*t8wSThN56mrG-q2smhNjYA.png", "Fungsi add di modul Calc" %}_Fungsi add di modul Calc_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*LAU7t8wv9JrjB2mIjZnPyg.png", "Memanggil modul Calc dan fungsi add dari modul Demo." %}_Memanggil modul Calc dan fungsi add dari modul Demo._
...
{% image "https://cdn-images-1.medium.com/max/2696/1*a-hgRCsAQNb1WktCrl7mIw.png", "Opps, error!" %}_Opps, error!_

Wah, ada _error_ di file Demo.bs.js di baris ke-empat. Mari kita lihat hasil konversinya.

{% image "https://cdn-images-1.medium.com/max/2200/1*X1w2p1gQUCcFNBUmVjmLUg.png", "Error require di file Demo.bs.js" %}_Error require di file Demo.bs.js_
...
{% image "https://cdn-images-1.medium.com/max/2112/1*T9_NyXF8Gam1Z5-PuhZiNQ.png", "Di NodeJS aplikasi berjalan normal" %}_Di NodeJS aplikasi berjalan normal_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*6IHWwR3-_poLDzukl0Vs0g.png", "File index.html" %}_File index.html_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*JKgqgN1dSIHGnUKPB3AIHg.png", "Placeholder index.re" %}_Placeholder index.re_
...
{% image "https://cdn-images-1.medium.com/max/2200/1*9ExqMLUETZQH39UDGilbCQ.png", "Konfigurasi bsconfig.json" %}_Konfigurasi bsconfig.json_
...
{% image "https://cdn-images-1.medium.com/max/4288/1*aB5Vpu42Gvtd1eTqqu2x8A.png", "Package bs-platform tidak ditemukan" %}_Package bs-platform tidak ditemukan_
...
{% image "https://cdn-images-1.medium.com/max/2000/1*FeU8LIKqPfWwkBslCb5DOw.png", "Parcel sudah berhasil menjalankan project Reason!" %}_Parcel sudah berhasil menjalankan project Reason!_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*2MPbvFjQLPv59EIt9f2Exw.png", "Modul Calc dengan fungsi add, dengan type annotation" %}_Modul Calc dengan fungsi add, dengan type annotation_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*caekOhKGcbdLlZrhqjqzSg.png", "Melakukan pemanggilan Modul Calc dan fungsi add." %}_Melakukan pemanggilan Modul Calc dan fungsi add._

Hasilnya dapat kita nikmati di console browser seperti berikut.

{% image "https://cdn-images-1.medium.com/max/2000/1*GYlHR2jiPuxSGRHcY5wB2A.png", "Hasil pemanggilan fungsi add." %}_Hasil pemanggilan fungsi add._
...
{% image "https://cdn-images-1.medium.com/max/2000/1*nYhg9HX7er4-1ejej1IVLg.png", "Definisi spesies DragonBall" %}_Definisi spesies DragonBall_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*PKmOQMieB9b6lNGUqpBVpw.png", "Tipe Record characters." %}_Tipe Record characters._

Setelah didefinisikan bentuk dan tipe datanya barulah kita dapat menggunakan record.

{% image "https://cdn-images-1.medium.com/max/2200/1*N25teCKUFx-S0gZHLcfNjw.png", "Menggunakan record Bejita dengan tipe record characters." %}_Menggunakan record Bejita dengan tipe record characters._
...
{% image "https://cdn-images-1.medium.com/max/2200/1*LuERyXDd1HHbHEsWN5utOg.png", "Mencocokkan berdasarkan species." %}_Mencocokkan berdasarkan species._
...
{% image "https://cdn-images-1.medium.com/max/2200/1*pLlJAkEbEfrdsK_xD5leww.png", "Support Other species" %}_Support Other species_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*5gpTjelFVz5Hy3IAqJ7xQQ.png", "Interopabilitas dengan bs.raw" %}_Interopabilitas dengan bs.raw_

Kita juga bisa menulis sintaks JavaScript yang setelah dievaluasi kemudian di _binding_ ke variable Reason.

{% image "https://cdn-images-1.medium.com/max/2240/1*xqMqys9wJyGVdUmbei8q4w.png", "Dari JavaScript di binding ke Reason" %}_Dari JavaScript di binding ke Reason_
...
{% image "https://cdn-images-1.medium.com/max/2240/1*uRjDitMfTOiCAiTclrqIXQ.png", "Menggunakan fungsi Math.PI JavaScript dan digunakan di Reason." %}_Menggunakan fungsi Math.PI JavaScript dan digunakan di Reason._
